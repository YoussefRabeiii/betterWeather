...
export function createClouds(
	...
	return {
		setOptions(next) {
			let changed = false;
			for (const key of Object.keys(next) as (keyof CloudsOptions)[]) {
			...
			// Skip resize/restart when nothing meaningful changed — prevents city-click flicker.
			if (!changed) return;
			syncCanvasSize();
			syncBaseColor();
			start();
		...
		destroy() {
			...
			try {
				...
				// Context may already be gone during fast effect swaps.
			}
		},
	};
}
