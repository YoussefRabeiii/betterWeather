...
export function WeatherGlobe({
	...
	useEffect(() => {
		if (typeof focusLat === "number" && typeof focusLon === "number") {
			...
			const spin = reducedMotionRef.current ? 0 : AUTO_SPIN;
			velocityRef.current = { phi: spin, theta: 0 };
		}
	}, [focusLat, focusLon]);

	useEffect(() => {
		const spin = reducedMotion ? 0 : AUTO_SPIN;
		velocityRef.current = { phi: spin, theta: 0 };
	}, [reducedMotion]);
	...
	useEffect(() => {
		...
		const globe = createGlobe(canvas, {
			...
			markers: MAJOR_CITIES.map((city) => ({
				...
				size: 0.055,
				color:
					colorsRef.current.pinColors[city.id] ?? colorsRef.current.markerColor,
				id: city.id,
			})),
		});
		...
		const animate = () => {
			...
			const autoSpin = reduce ? 0 : AUTO_SPIN;

			if (target && !pointerRef.current.dragging) {
				if (reduce) {
					phiRef.current = target[0];
					thetaRef.current = clampTheta(target[1]);
				...
				if (reduce) {
					...
					const momentum = Math.abs(vel.phi - autoSpin) + Math.abs(vel.theta);
					...
					if (momentum > COAST_EPS) {
						...
						vel.phi += (autoSpin - vel.phi) * (0.04 + fade * 0.12);
						if (Math.abs(vel.theta) < COAST_EPS) vel.theta = 0;
					} else {
						vel.phi = autoSpin;
						vel.theta = 0;
						phiRef.current += autoSpin;
					}
				}
			}

			frame += 1;
			const pulse = reduce ? 0.08 : 0.08 + Math.sin(frame / 12) * 0.015;
		...
		const onPointerMove = (e: PointerEvent) => {
			...
			if (!reducedMotionRef.current) {
				...
				velocityRef.current.phi =
					velocityRef.current.phi * 0.55 + samplePhi * 0.45;
